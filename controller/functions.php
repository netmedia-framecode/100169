    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkEmail) > 0) {
        ...
        if ($data['password'] !== $data['re_password']) {
          ...
          $to       = $data['email'];
          $subject  = "Account Verification - SI Inventaris Sekolah";
                        ...
                        <tr>
                            ...
                            <table role='presentation' border='0' cellpadding='0' cellspacing='0' style='border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; min-width: 100%; width: 100%;' width='100%'>
                                ...
                                <td style='font-family: sans-serif; font-size: 14px; vertical-align: top;' valign='top'>
                                    ...
                                    <p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;'>Terima kasih telah mendaftar di SI Inventaris Sekolah.</p>
  ...
  function forgot_password($conn, $data, $action, $baseURL)
  {
    if ($action == "update") {
      ...
      if (mysqli_num_rows($checkEmail) === 0) {
        ...
        require_once("mail.php");
        $to       = $data['email'];
        $subject  = "Reset password - SI Inventaris Sekolah";
        $message  = "<!doctype html>
        <html>
          <head>
              <meta name='viewport' content='width=device-width'>
  ...
  function sub_menu($conn, $data, $action, $baseURL)
  {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkSubMenu) > 0) {
        ...
        $_SESSION["project_si_inventaris_sekolah"]["name_page"] = "' . $data['title'] . '";
